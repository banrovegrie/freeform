...
characterization is tight and model-independent. Farhi,
Goldstone, Gutmann, and Nagaj showed that the same $\Omega(\sqrt{N/d_0})$
barrier persists under continuous-time Hamiltonian
evolution~\cite{farhi2008fail}. The
exponent is settled.
...
adiabatic algorithm matching the same scaling faces a different challenge. It
requires a schedule adapted to the spectral gap profile along a Hamiltonian
...
be, in quantitative terms and not just qualitative ones? Can the resulting algorithm match
...
\end{equation}
The factor $1/T$ in front makes the dependence on total runtime explicit.
Larger $T$ slows the effective rate of change and improves adiabatic tracking. The
...
Aharonov, van~Dam, Kempe, Landau, Lloyd, and
...
drivers, which are the standard choice in quantum annealing hardware, can produce
...
The qualitative adiabatic theorem is simple. Go slowly enough and you stay
near the ground state. For algorithm design, ``slowly enough'' must be a number, not an
...
application, this is the instantaneous ground-state subspace. Assume it has
...
$O(N)$. This is no better than classical exhaustive search. The linear
...
bottleneck is a narrow avoided crossing of width $\delta_s$ where
...
the gap dependence from inverse-square to inverse-linear, provided the
Lebesgue measure of $\{s : g(s) \leq t\}$ grows at most linearly in
...
spectral inference are therefore entangled problems. The sequential approach
of first computing the gap and then designing the schedule is valid only
...
In the rank-one AQO setting, the gap profile along the interpolation path is wide at both endpoints
and pinches to a minimum
...
structural motif is the same. In both settings, low-dimensional effective
dynamics controls an exponentially large system. The parameterization changes from a
...
narrow dip. The location, width, and minimum value of the crossing are all
determined by $N$ alone. No instance-specific information is needed.
...
$T = \Omega(\sqrt{N/d_0})$~\cite{farhi2008fail}. Their proof adapts the BBBV argument from Chapter~3 to continuous time,
introducing $N$ auxiliary Hamiltonians related by diagonal phase shifts and
bounding the average distinguishability of the resulting final states. The Roland-Cerf
...
adiabatic evolution beyond optimization, applying it to state generation,
Markov-chain speedups, and
...
The parameter dependencies reveal the physics. A larger $d_0$ means more states
share the ground energy, which widens the crossing and increases $g_{\min}$
by concentrating more amplitude in the ground subspace. A larger $A_2$ means
excited eigenvalues sit closer to the ground energy, which narrows the gap as
...
precision $\varepsilon < 1/(72(n-1))$ is NP-hard, by reduction from
satisfiability. The construction encodes 3-SAT instances into the
...
describe its structure. But the spectral moments $A_1$ and $A_2$ that the schedule requires can
...
algorithm matching the same runtime requires the gap profile, the crossing
location, and the schedule, all of which depend on the full spectrum of the
problem Hamiltonian. The circuit model achieves its frontier

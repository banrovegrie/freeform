...
The argument requires three layers of preparation before reaching the main
results, and each concept earns its place by answering a question the previous
one left open. A cost function becomes a diagonal Hamiltonian, optimal
solutions become ground states, and the classical hierarchy of P, NP, and $\#$P constrains what any
computational procedure can achieve.
Hamilton's equations generate the dynamics, the Gibbs distribution governs
thermal equilibrium, and the Schr\"odinger equation carries the system into a
quantum regime where the computational model must change. The chapter closes
with the adiabatic mechanism as a physical heuristic, stopping short of
quantitative guarantees (Chapter~2).
...
computational task (Chapter~9). The hardness reductions are formalized in
Lean~4 in the appendix.

...
Vladimir Voevodsky pushed this further than anyone expected. A Fields medalist in algebraic geometry, he discovered in the mid-2000s that a key result he and others had relied on contained an error that had survived years of peer review \cite{voevodsky2014origins}. He found further errors in his own earlier work. The experience convinced him that the informal verification system (peer review, reputation, careful reading) was not reliable enough for the mathematics he cared about. He turned to formalization, and he found something remarkable. The identity types in Martin-L\"of's system, the types that express ``$a$ equals $b$'', turned out to have geometric structure. Two proofs that $a = b$ are not necessarily the same, just as two paths between two points in a space are not necessarily homotopic. Types behave like spaces. Terms are points. Identity proofs are paths. Higher identity proofs are homotopies. This is homotopy type theory \cite{hottbook2013}. Voevodsky's univalence axiom says that equivalent mathematical structures are identical: if two types are equivalent, the identity type between them is inhabited. Theorems proved about one representation transport automatically to any equivalent representation.

Lean 4 does not use homotopy type theory. Its foundation is the Calculus of Inductive Constructions with quotient types and proof irrelevance, a different branch of the same family tree \cite{moura2021lean4}. But the practical lessons carry over. Propositions are types. Proofs are terms. Dependent types package invariants into interfaces. And the bridge theorems in this development, like \texttt{A1\_partial\_eq\_A1} where two representations of a spectral parameter are proved equal and downstream code uses whichever is convenient, are the working version of Voevodsky's insight: equivalent representations should be interchangeable, and the system should enforce it. Lean's trusted kernel is small enough to audit independently. Mathlib \cite{mathlib2020}, the community-maintained library, provides the mathematical infrastructure that makes large developments practical.

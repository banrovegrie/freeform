...
that gap for a specific computational task by arranging interference so that the
...
$\ket{1}$, the two basis states of a quantum bit, or qubit. The state space of a single qubit is
\begin{equation}
...
The culprit is the tensor product. A two-qubit system does not merely carry the state of
...
identical statistics when measured in the $\{\ket{0},\ket{1}\}$ basis. Both
yield $0$ or $1$ with probability $1/2$. Yet they are perfectly
...
space. This rotation acts not in physical space but in the abstract space of
...
it, one needs a formal model of computation that specifies what counts as
...
length $n$. The choice of $2/3$ is arbitrary in the sense that any constant
bounded away from $1/2$ defines the same class. Standard repetition with
...
\end{equation}
Optimization (finding $\arg\min_x C(x)$ in this context) reduces to repeated calls to
...
\end{equation}
When $d_0 \ll N$, the initial state $\ket{s}$ is nearly orthogonal to the
target $\ket{w}$, tilted toward it by an angle $\theta$ of order
$\sqrt{d_0/N}$.
...
unstructured search. Bennett, Bernstein, Brassard, and Vazirani showed that
no quantum algorithm can do better~\cite{bennett1997strengths}.
...
\end{equation}
This is not merely a known algorithm paired with a known lower bound. The
quantum query complexity of unstructured search is now determined up to
constant factors. No future algorithm can improve the exponent.
...
The word ``unstructured'' carries precise but different meanings in the two
settings. In the circuit model, it means black-box oracle access. The only way to learn
about $f$ is to query it, and no exploitable regularity in the labels is
promised or assumed. In the adiabatic model, it means that the problem Hamiltonian is a
diagonal cost map $z \mapsto E_z$ with no additional promised structure and no
...
only knowledge of $N$ and $d_0$. The algorithm runs $k^*$ iterations of a fixed

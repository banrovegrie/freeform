...
communication cost for each target runtime.

The formalization in Lean~4 covers the hardness reductions and spectral
parameter identities with 330 named theorems and 15 explicit axioms. The axiom
budget is inspectable by any reader who rebuilds the artifact, and each axiom
is a named interface ready to be discharged as Lean's mathematical libraries
grow.
...
The Lean~4 formalization covers the complexity reductions but not the spectral
analysis. Formalizing the resolvent calculus behind the gap bounds of
Chapters~5 and~6 would close the largest remaining axiom group
(\texttt{FullSpectralHypothesis}) and complete the chain from Hamiltonian
definition to runtime bound within a single typed proof system. The nearest
target is the Sherman-Morrison identity for rank-one perturbations and the
monotonicity of the resolvent function $f(s)$, both of which reduce to
Mathlib-compatible linear algebra. The Garey-Johnson encoding axiom, which
bridges the classical satisfiability reduction to the Hamiltonian formulation,
is another natural discharge target.

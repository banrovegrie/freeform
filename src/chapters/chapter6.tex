...
where $\dot{H} = H_z + \ket{\psi_0}\bra{\psi_0}$ and $\ket{\phi_j(s)}$ are the instantaneous eigenstates. Concavity means any chord lies below the function: for $s < s^*$, the value $\lambda_0(s)$ is at most the linear interpolation between $\lambda_0(0)$ and $\lambda_0(s^*)$. Since $\lambda_0(s^*) < s^*E_0$, this yields a bound of the same form as~\eqref{eq:lambda0-upper}, though the variational approach gives slightly sharper constants.
...
\end{equation}
Bounding the gap from below reduces to bounding the resolvent norm from above. This resolvent approach to rank-one perturbations has precedent in the spatial search literature, where Childs and Goldstone~\cite{childs2004spatial} and Chakraborty et al.~\cite{chakraborty2016spatial, chakraborty2020optimality} used it to analyze continuous-time quantum walks on graphs. The adiabatic Hamiltonian shares the same structure --- a projector perturbation of a diagonal operator --- making these techniques directly applicable.
...
\end{equation}
where $\epsilon_0 = d_0/N$ denotes the fraction of ground states among the $N$ computational basis states, and
\begin{equation}
...
\textbf{Monotonicity of $f$.} We show $f'(s) < 0$ for $s \in [s^*, 1]$. Writing $f = u/v$, the sign of $f'$ is determined by $u'v - uv'$. After expanding and cancelling common terms, the expression reduces to three contributions: two are manifestly negative, while a third --- positive and proportional to $\epsilon_0\,s_0$ --- arises from having shifted the line's origin below $s^*$. The proof amounts to showing the negative terms dominate. Write $f = u/v$ with
\begin{align}
\label{eq:uv-def}
...
\end{equation}
Replacing $g_{\min}$ by its leading-order expression $\hat{g} = 2s^*\sqrt{\epsilon_0/A_2}$ from Eq.~\eqref{eq:gmin-formula} (valid up to a $(1 \pm O(\eta))$ factor that does not affect the final constant), we have $A_2 k^2 \hat{g}^2 = 4k^2{s^*}^2\epsilon_0$. Substituting:
\begin{equation}
\label{eq:f-at-sstar}
...
This chapter established the piecewise linear gap profile (\autoref{thm:complete-profile}) that controls the adiabatic runtime. The variational principle yields a tight bound to the left of the crossing, with a slope determined by $A_1$ and $A_2$ alone. The resolvent approach, combined with the Sherman-Morrison identity for rank-one perturbations, yields a uniform bound to the right, with a shallower slope controlled by the spectral gap $\Delta$ of the problem Hamiltonian. The two techniques are complementary: the variational method exploits the simplicity of the spectrum below the crossing, while the resolvent method handles the congested spectrum above it. Together with the window bound from Chapter 5, they determine the runtime integral up to constant factors, which Chapter 7 evaluates to obtain the optimal adiabatic schedule. The gap profile also reveals which spectral parameters matter for hardness: $g_{\min}$ depends on $d_0$, $A_1$, and $A_2$, while the slope of the right bound depends additionally on $\Delta$. Chapter 8 exploits this dependence to establish computational hardness of optimizing the schedule.

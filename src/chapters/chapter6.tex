...
The same bound follows from concavity. The second derivative of $\lambda_0(s)$ satisfies $\ddot{\lambda}_0(s) = -2\sum_{j \geq 1} |\bra{\phi_j(s)}\dot{H}\ket{\phi_0(s)}|^2/(E_j(s) - E_0(s)) \leq 0$, where $\dot{H} = H_z + \ket{\psi_0}\bra{\psi_0}$ and $\ket{\phi_j(s)}$ are the instantaneous eigenstates. The tangent to the concave function $\lambda_0(s)$ at $s = s^*$ lies above it everywhere, and its slope evaluated using Eq.~\eqref{eq:lambda0-upper} reproduces the same linear bound.

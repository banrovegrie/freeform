...
The paper proves two hardness results for approximating $A_1$. At inverse-polynomial precision ($\varepsilon < 1/\mathrm{poly}(n)$), the problem is NP-hard via a local Hamiltonian reduction (Theorem 2). At exponential precision ($\varepsilon = O(2^{-\mathrm{poly}(n)})$), the problem is #P-hard via polynomial interpolation and Paturi's lemma (Theorem 3). But the adiabatic algorithm needs $A_1$ at precision $\varepsilon = 2^{-n/2}$, which falls strictly between these two regimes.
...
The polynomial interpolation technique of paper Section 3.2 requires precision $\varepsilon = 2^{-n - O(M \log n)}$ to extract exact degeneracies. At $\varepsilon = 2^{-n/2}$, the amplified error exceeds $1/2$ and rounding fails. The #P-hardness argument does not extend to precision $2^{-n/2}$.

### Theorem 2 (Quantum Algorithm for $A_1$)
There exists a quantum algorithm estimating $A_1$ to precision $\varepsilon$ using $O(\sqrt{N} + 1/(\varepsilon \cdot \Delta_1))$ queries. For $\varepsilon = 2^{-n/2}$: $O(2^{n/2} \cdot \mathrm{poly}(n))$.

### Theorem 3 (Classical Query Lower Bound)
Any classical algorithm estimating $A_1$ to precision $\varepsilon$ requires $\Omega(1/\varepsilon^2)$ queries. At $\varepsilon = 2^{-n/2}$: $\Omega(2^n)$.
...
The quantum query complexity of $A_1$ estimation to precision $\varepsilon$ is $\Omega(1/\varepsilon)$, via reduction from approximate counting and the Heisenberg limit for quantum phase estimation. Combined with Theorem 2: the quantum query complexity at $\varepsilon = 2^{-n/2}$ is $\Theta(2^{n/2})$. Both quantum and classical bounds are now tight:
...
Any polynomial extrapolation scheme of degree $d$ that evaluates outside the interpolation interval has Lebesgue function $\Lambda_d(x^*) \ge 2^{d-1}$. This amplification is inherent: no rearrangement of interpolation nodes, no alternative polynomial basis, and no change of variables can circumvent it. The paper's specific construction is not uniquely vulnerable; the entire class of polynomial-interpolation-based reductions fails at $\varepsilon = 2^{-n/2}$.

### Theorem 7 (Structure Irrelevance)
For every $M \ge 2$ and every gap structure, there exist diagonal Hamiltonians for which $A_1$ estimation to precision $\varepsilon$ requires $\Omega(1/\varepsilon)$ quantum queries and $\Omega(1/\varepsilon^2)$ classical queries. The sum-of-reciprocals structure of $A_1$ provides no worst-case advantage over generic mean estimation: $M = 2$ instances (where $A_1$ reduces to approximate counting) are the hardest case.

### Proposition 8 (Precision Phase Diagram)
Complexity as a function of precision $\varepsilon$ is now explicit. In the core query family, complexity scales continuously as $\Theta(1/\varepsilon)$ quantum and $\Theta(1/\varepsilon^2)$ classical. Regime values:
- $\varepsilon = \Theta(1)$: trivial constant-precision approximation
- $\varepsilon = 1/\mathrm{poly}(n)$: NP-hard computationally (paper Theorem 2)
- $\varepsilon = 2^{-cn}$, $0 < c < 1/2$: quantum $\Theta(2^{cn})$, classical $\Theta(2^{2cn})$
- $\varepsilon = 2^{-n/2}$: quantum $\Theta(2^{n/2})$, classical $\Theta(2^n)$
- $\varepsilon = 2^{-cn}$, $c > 1/2$: same query scaling, and #P-hard once $cn$ exceeds the paper's interpolation threshold; below that threshold the computational status remains open
- $\varepsilon = 2^{-\mathrm{poly}(n)}$: #P-hard (paper Theorem 3)
...
At precision $\varepsilon$, the natural containment is parameterized:
$\mathrm{A1\mbox{-}EST}_\varepsilon \in \mathrm{FBQTIME}(\sqrt{N} + 1/(\varepsilon\Delta_1)\cdot \mathrm{poly}(n))$ (Theorem 2), with core lower bound $\Omega(1/\varepsilon)$ (Theorem 4). At $\varepsilon = 2^{-n/2}$ this gives $\mathrm{FBQTIME}(2^{n/2}\mathrm{poly}(n))$, so the natural description is parameterized-time rather than PromiseBQP-type polynomial-time behavior in the query model.


## Complete Complexity Landscape

**Query complexity at $\varepsilon = 2^{-n/2}$** (tight, for $\Delta_1 = \Theta(1)$):
...
| Model | Complexity in $\varepsilon$ | Source |
|-------|---------------------------|--------|
| Quantum | $\Theta(1/\varepsilon)$ | Thm 4 + Thm 2 (core family) |
| Classical | $\Theta(1/\varepsilon^2)$ | Thm 3 + sampling upper bound |
...
**Proof technique barrier**: Polynomial extrapolation requires precision $2^{-\Omega(d)}$ for degree-$d$ interpolation (Thm 6). At $d = \mathrm{poly}(n)$, the required $\varepsilon = 2^{-\Omega(n)}$ is exponentially below $2^{-n/2}$.
...
2. Can a non-interpolation reduction establish #P-hardness at $\varepsilon = 2^{-n/2}$? (Theorems 1 and 6 rule out polynomial extrapolation only.)
...
4. Can one prove tight computational lower bounds for the quantum side (not only query lower bounds), matching the $\Theta(1/\varepsilon)$ query law?
...
- **Experiment 04 (separation theorem):** The $\varepsilon = 2^{-n/2}$ point gives the concrete numerical bridge between ignorance penalties and computable schedule information.
...
1. **Subsection: Precision Phase Diagram for $A_1$**
   ...
   Message: query scaling is smooth in $\varepsilon$, but proof techniques bifurcate at
   $\varepsilon = 2^{-n/2}$.
...
3. **Generic proof barrier** (Theorems 1, 6): Polynomial extrapolation inherently requires $\varepsilon = 2^{-\Omega(n)}$. New proof ideas needed for #P-hardness at $2^{-n/2}$.
...
**Open problem note**: Directly addresses the paper's explicit open problem (Discussion, p.983; Section 3.2, p.962). The answer is now two-layered: query complexity varies smoothly with precision, but $\varepsilon = 2^{-n/2}$ is the structural boundary where interpolation-based #P-hardness arguments fail and schedule-level information cost becomes explicit.
...
1. Core grid scaling: $\varepsilon_k = 2^{-k}$, $Q(\varepsilon_k)=2^k$, $C(\varepsilon_k)=2^{2k}$, $C/Q=2^k$, with one-bit refinement laws $Q_{k+1}=2Q_k$ and $(C/Q)_{k+1}=2(C/Q)_k$.

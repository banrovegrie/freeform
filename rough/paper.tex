...
$$
such that $\ket{\psi(0)}$ is a ground state of $H(0)=H_0$ and $0\leq s\leq 1$, and $T$ is the total evolution time. Furthermore, let $P(s)$ be the projection on to the space spanned by the ground states of $H(s)$, which has degeneracy $d$, and $g(s)$ be the spectral gap of $H(s)$. Then, there exists a constant $C$ such that the quantum adiabatic theorem satisfies
$$
...
$$
\varepsilon \leq \frac{1}{72}\cdot\frac{1}{n-1},$$ 
...
\end{equation}
Note that the eigenvalues of $H$ lie between $0$ and $1$. We aim to disambiguate between $E_0 = 0$ and $E_0 \geq 1/6m$ using Lemma \ref{lem:distinguish-ground-energy-general}.
...
%\footnote{3-\SAT\ with $m$ clauses can be mapped to an instance of the Maximum-Weight Independent Set (MIS) problem in a graph with $3m$ nodes, where the underlying decision problem asks if there is a MIS with $m$ nodes. The counting version would then require us to define an Ising Hamiltonian whose degeneracy of the ground state counts the number of solutions of an MIS in a graph of $3m$ nodes.}.  
...
Observe that $\delta_0^\pm$ are the roots of the quadratic equation formed by ignoring the terms in $f(\delta)$, and thus we show that $|f(\delta)|$ is upper-bounded by some small constant so that there exists a positive constant $\eta<0.5$ for which $F$ is changing sign going from $(1-\eta)\delta_0^\pm$ to $(1+\eta)\delta_0^\pm$.

%Since $\Delta$ is the spectral gap of $H_z$, we have that for all any integers $p\ge q\geq 1$,
...
\begin{align}
    ...
    &\leq  s\cdot \Delta \cdot c \cdot\kappa \qquad \text{using Eq.~\eqref{eq:hamiltonian-constraint}}\label{eq:upper-bound-delta_0}
\end{align}
%Here, we used Eq.~\eqref{eq:hamiltonian-constraint} with the fact that 
%$$\frac{(A_1+1)^2}{2A_2}\delta_s =  \sqrt{\frac{d_0}{A_2N}}<\Delta c.$$ 
where we used that $\frac{\delta_s}{1-s^*} \leq 2c$ (shown in the proof of Lemma \ref{lem:spectral-gap-in-robustness-window}) and we introduced
$$
\kappa = \left(\dfrac{1+\sqrt{1+(1-2c)^2}}{(1-2c)}\right),
$$
such that $c\kappa$ with a value of $c<0.2424$ ensures that $c\kappa<1$. This leads to
\begin{equation}
...
\noindent We want to show that for some $\eta \leq 0.5$, $F(\delta_0^\pm(1+\eta)$ is changing sign when $\eta$ changes sign as well. Ignoring the $1/\delta$ factor in Eq.~\eqref{eq:expression-F-delta} and using the property that $\delta_0^\pm$ is the root of the quadratic function, we have: 
\begin{align*}
...
\end{align}
We observe from Eq.~\eqref{eq:inter-upper-delta0} that $\dfrac{{\delta_0^+}^2}{s^2}\leq \dfrac{d_0}{NA_2}\kappa^2$. So Eq. (\ref{eq:boundFpos}) gives us:
\begin{align*}
...
\noindent If we fix $\eta = 0.1$, the first condition gives $c\leq 0.022$ and the second $c\leq 0.034$.

From this we conclude that the roots of $F(\delta)=0$ exist in the intervals \begin{align*}
&\delta_+(s)\in\left((1-\eta)\delta^{+}_{0}(s),~(1+\eta)\delta^{+}_{0}(s)\right),~\text{ and }\\
&\delta_-(s)\in\left((1+\eta)\delta^{-}_{0}(s),~(1-\eta)\delta^{-}_{0}(s)\right),
\end{align*}
and
...
\item[$(2)$]~$\lVert P^{\prime\prime}\rVert \leq 8 \dfrac{\lVert H'\rVert^2}{g^2} + 2 \dfrac{\lVert H^{\prime\prime}\rVert}{g}$;

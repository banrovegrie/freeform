...
% \title{
%  Limitations on Grover-like speedups in Adiabatic Quantum Optimization (I think we need to improve the title)
% }
\title{
...
~\\~\\
\textbf{The complexity class \textbf{\sharpP}:~}The class \sharpP, introduced by Valiant \cite{valiant1979permanent, valiant1979complexity}, can be seen as the counting analogue of the well-known complexity class~\NP. For problems in \NP, for any given input, we ask whether there is a solution, i.e.\ the output is binary $(0/1)$ (decision problem). On the other hand, in the case of counting problems, for any given input, the output is a natural number denoting the number of solutions. We formally state the definition of this class below: 
...
Much like \NP\, it is possible to analogously define the complexity classes \sharpP-hard\ and \sharpP-complete. Problems in this class are also hard for a classical computer to solve efficiently.
~\\~\\
...
$$
such that $\ket{\psi(0)}$ is the unique ground state of $H(0)=H_0$ and $0\leq s\leq 1$, and $T$ is the total evolution time. Furthermore, let $P(s)$ be the projection on to the space spanned by the ground states of $H(s)$, which has degeneracy $d(s)$, and $g(s)$ be the spectral gap of $H(s)$. Then, the quantum adiabatic theorem satisfies
$$
\left|1-\braket{\psi(s)|P(s)|\psi(s)}\right|\leq \gamma^2(s),
$$
where
$$
\gamma(s)= \dfrac{4}{T}\dfrac{d(0)\norm{H(0)}}{g(0)^2}+\dfrac{4}{T}\dfrac{d(s)\norm{H'(s)}}{g(s)^2}+\dfrac{1}{T}\int_{0}^{s} \left( \dfrac{4~d(s)\norm{H''(s)}}{g(s)^2}+ \dfrac{56~ d^{3/2}(s)\norm{H'(s)}}{g(s)^3}\right)~ds.
$$
\end{lemma}

This lemma provides a precise lower bound on the total evolution time $T$ of the underlying AQC. If, at the end of the adiabatic evolution, we wish to end up in a state that is $\varepsilon$-close in trace distance to the underlying ground states, it is enough to make sure that $\gamma^2(1)=\varepsilon$, which can be achieved by choosing some $T$ which is a polynomial in the inverse of the minimum spectral gap of any intermediate Hamiltonian along the adiabatic path, and a polynomial in $1/\varepsilon$. That is,  
$$
...
\end{equation}
where $J_{ij},~h_j \in \{-m,~-m+1,~\ldots~,~0,~1, \ldots,~m-1,~m\}$ for some constant positive integer $m\in \Theta(1)$. There are $M$ distinct eigenvalues of $H_{\sigma}$, all of which are integers and $M\in \poly(n)$. This is the quantum version of Ising spin glass Hamiltonians, and solutions to several \NP-hard (or \NP-complete) problems can be encoded in the ground states of $H_{\sigma}$. For details, we refer the readers to Refs.~\cite{barahona1982computational, lucas2014ising}. Note that the normalized version of $H_{\sigma}$ satisfies the conditions outlined in Definition \ref{def:prob-Ham}. In particular, it has a spectral gap $\Delta\geq 1/\poly(n)$, and moreover, the quantity $A_2 \geq 1/\norm{H_{\sigma}}^2$, which implies that the condition in Eq.~\eqref{eq:spectral-condition} holds. Nevertheless, we shall keep our analysis general as it works for a broader class of local spin Hamiltonians. We shall invoke particular cases (such as the classical Ising Hamiltonian) only when necessary.

\subsection{Summary of our results}
\label{subsec:results}

In this section, we state the main results of this work and outline the techniques central to their derivation. We consider the time-dependent adiabatic Hamiltonian
$$
H(s)=-(1-s)\ket{\psi_0}\bra{\psi_0}+ s H_z,
$$
where $H_z$ is as stated in Definition \ref{def:prob-Ham}. AQO starts from $\ket{\psi_0}$ (the ground state of the initial Hamiltonian) and adiabatically evolves into a quantum state that has a fidelity of at least $1-\varepsilon$ with the ground states of $H_z$. 
\begin{figure}[h!]
    \centering
...
We provide an approximation of the position of the avoided crossing between the ground and the first excited states. We find the point where the spectral gap of $H(s)$ is minimum and identify a narrow window of $s$, where the spectral gap scales similarly to the minimum gap. Indeed, we prove that the position of the avoided crossing is well approximated by 
\begin{equation}
\label{eq:position-ac}
s^*=\dfrac{A_1}{A_1+1}.
\end{equation}
In fact, the spectral gap of $H(s)$ (denoted by $g(s)$) remains close to the minimum gap within a small window $\delta_s$ around $s^*$ (See inset Fig.~\ref{fig:spectrum-H(s)}). That is, the actual position of the avoided crossing is within the interval $\mathcal{I}_{s^*}=[s^*-\delta_s, s^*+\delta_s]$, where
\begin{equation}
...
In this work, we rigorously investigate how difficult it is to compute $A_1$. More precisely, suppose we have access to an adiabatic quantum optimizer and a classical computing device. Then, what is the computational hardness of exactly, and subsequently, approximately estimating $A_1$? 
We prove that it is \NP-hard to approximate $A_1$ even to within an additive precision of $1/\poly(n)$ (which is much larger than the desired accuracy) for a 3-local problem Hamiltonian. To this end, we consider 3-\SAT, a well-known \NP-complete problem. This problem asks if a given Boolean formula has a satisfying assignment, and solving it reduces to whether we can disambiguate between two promised thresholds of the ground energy of a 3-local Hamiltonian $H$. We are able to prove that it is possible to distinguish between these two thresholds of the ground energy by making only two calls to any classical algorithm that estimates $A_1$ to an additive accuracy of $1/\poly(n)$. Formally, we prove the following lemma:
~\\
...
In this work, we rigorously investigate how difficult it is to compute $A_1$. More precisely, suppose we have access to an adiabatic quantum optimizer and a classical computing device. Then, what is the computational hardness of exactly, and subsequently, approximately estimating $A_1$? We prove that the problem of estimating $A_1$ exactly (or approximately, within an accuracy of $2^{-n}$) is as hard as solving any problem in \sharpP, i.e.\ it is \sharpP-hard. For this, consider a classical procedure $\mathcal{C}$ that exactly estimates $A_1$, then we prove that it is possible to extract the degeneracies $d_k$ in the spectrum of the Ising Hamiltonian $H_{\sigma}$ using only $O(\poly(n))$ calls to $\mathcal{C}$. The idea is to modify $H_{\sigma}$ by adding an extra spin qubit of a certain local energy and use $\mathcal{C}$ to estimate $A_1$ for this modified Hamiltonian. By varying this local energy term of the additional spin, we estimate $A_1$ for $O(\poly(n))$ different values, requiring only $O(\poly(n))$ calls to $\mathcal{C}$. Then, by using standard polynomial interpolation techniques, we are able to construct a polynomial from which the degeneracies $d_k$  of $H_{\sigma}$ can be extracted exactly. 

Recall that solutions to \NP-hard problems can be encoded in the ground states of $H_{\sigma}$, including 3-\SAT. The counting version of this problem (\#3-\SAT) asks the number of satisfying assignments of a given Boolean formula and is \sharpP-complete. This is equivalent to extracting the degeneracy of the ground state $d_0$, of $H_{\sigma}$. Thus, our reduction proves that computing $A_1$ exactly is as hard as solving any problem in the class \sharpP, and hence is \sharpP-hard. As a side note, another route of proving \sharpP\ hardness is that extracting the degeneracies $d_k$ of $H_{\sigma}$ allows us to estimate the output probability of an IQP circuit, which is known to be \sharpP-hard.

What if we allow for a sufficiently small error in the estimation of $A_1$? We use Paturi's lemma \cite{paturi1992} to bound the errors of the polynomial we construct in the exact case. Thus, our hardness result is robust as long as the classical procedure $\mathcal{C}$ estimates $A_1$ to an accuracy of $\varepsilon\in O(2^{-\poly(n)})$. Formally, we state both these results succinctly via the following Theorem: 
\begin{theorem}[Main Result 2: Hardness of estimating $A_1$]
\label{thm:main-result-2}
Suppose there exists a classical algorithm $\mathcal{C}(\langle H\rangle)$ which accepts as input the description of an $n$-qubit Hamiltonian $H$, and outputs $A_1$ exactly, or with an accuracy of $\varepsilon$ such that $\varepsilon\in O(2^{-\poly(n)})$. Furthermore, suppose $H_{\sigma}$ be the $n$-qubit Ising Hamiltonian (with appropriate parameter ranges, as defined in Eq.~\eqref{eq:Ising_Ham}), such that its eigenenergies are $E_0<E_1<\cdots E_{k}$, where $0\leq k \leq M-1$, with known gaps $\Delta_{k}=E_k-E_0$. Then for all $k\in [0, M-1]$, it is possible to estimate the degeneracy $d_k$ of energy eigenvalue $E_k$ by making $O(\poly(n))$ calls to $\mathcal{C}$. 
\end{theorem}
Additionally, we show that it is possible to also extend this proof to the probabilistic setting, i.e.\ when the classical procedure outputs an approximation of $A_1$ only with a certain (constant) probability. Then, by querying this classical procedure $k$ times, we obtain enough correct sample points corresponding to the polynomial to be constructed. Provided $k\in \poly(n)$ is sufficiently large, we can use the Berlekamp-Welch algorithm \cite{bouland2019complexity, movassagh2023hardness} to reconstruct the polynomial exactly. Then, Theorem \ref{thm:main-result-2} can be used to estimate the degeneracies with a high probability once again. These results provide evidence that it is hard to even approximate $A_1$, which needs to be done prior to running the adiabatic algorithm and hence is crucial to the optimality of AQO. 

However, we establish that it suffices to estimate the position of the avoided crossing to an accuracy of $O(\delta_s)$, which for the Ising Hamiltonian scales as $\widetilde{O}(2^{-n/2})$. Unfortunately, the hardness results in Theorem \ref{thm:main-result-2} do not hold such high values of precision. Nevertheless, we provide evidence that approximating $A_1$ to the desired precision in a time less than $T$ is also hard. Indeed, the best classical algorithms employ sampling techniques to approximate the solutions to \sharpP-complete problems, to additive accuracy $O(\delta_s)$ in time $O(1/\delta_s^2)$ \cite{gurvitz2005mixed}, which is significantly higher than $T$. Estimating $A_1$ (with a constant probability) to the desired precision using sampling techniques also requires at least $\widetilde{O}(2^n/d_0)$ samples. Furthermore, we prove that even if we relax the additive precision to $1/\poly(n)$, the problem of approximating $A_1$ is already \NP-hard. This also indicates that it is computationally hard to estimate this quantity to a precision of $O(\delta_s)$. As discussed earlier, this points to a fundamental limitation of the framework of AQO, which is absent in the circuit model, and we leave open the question of whether this can be circumvented without requiring access to a digital quantum computer.     
 
 \subsection{Related Work}
\label{subsec:prior-work}
As mentioned previously, the adiabatic model was originally envisioned as a framework for solving optimization problems by Farhi et al. \cite{farhi2000adiabatic,farhi2001adiabatic}. In this paradigm, referred to as adiabatic quantum optimization, typically the ground states of the problem Hamiltonian encode the solution to a combinatorial optimization problem (such as some \NP-hard problem). We consider any $n$-qubit Hamiltonian that is diagonal in the computational
basis (which includes classical Ising spin Hamiltonians that are known to encode computationally hard problems \cite{barahona1982computational, lucas2014ising}). Also, these Hamiltonians belong to the class of the so-called \textit{stoquastic Hamiltonians} (there exists a basis with respect to which all its off-diagonal elements are nonpositive) \cite{bravyi2008stoquastic}. Stoquastic Hamiltonians have been extensively studied from the perspective of the $k$-local Hamiltonian problem, resulting in interesting complexity-theoretic consequences \cite{bravyi2010complexity, jordan2010qma}. However these Hamiltonians are not universal for quantum computation. Indeed, for the universality of adiabatic quantum computation, it is necessary for the problem Hamiltonian to be non-stoquastic \cite{aharonov2007adiabatic}. AQC using stoquastic Hamiltonians may lead to superpolynomial \cite{Hastings2021powerofadiabatic} and sub-exponential \cite{gilyen2021subexponential} oracular separations over classical computation for certain contrived problems. 

In the setting of AQO, however, obtaining any quantum speedup is a challenge. In fact, it has been observed that for random instances of \NP-hard problems, gaps are super-exponentially small, requiring exponential time \cite{reichardt2004adiabatic,altshuler2010anderson}.
The question we ask in this paper is whether a generic quadratic speedup over classical brute force search can be obtained to solve computationally hard problems using AQO. For this, we consider $H(s)$ to be a linear interpolation between a one-dimensional projector and a problem Hamiltonian that is diagonal in the standard basis (such as a classical Ising spin Hamiltonian) with a large enough spectral gap.  In Ref.~\cite{horvat2006exponential}, using a combination of analytical and heuristic arguments, the authors showed that the minimum spectral gap of $H(s)$ is exponentially small for the 3-\SAT\ problem (it scales roughly as $\sqrt{d_0/2^n}$, where $d_0$ is the number of solutions). Farhi et al.\ demonstrated that the choice of the initial Hamiltonian is important to obtain significant speedups using AQO. It was proved therein that when the initial Hamiltonian is a one-dimensional projector, the running time of AQO would be at least $2^{n/2}$ \cite{farhi2008fail}. 

However, whether AQO could attain this lower bound for a broad class of optimization problems has been unknown. This is because it is difficult to bound the spectral gap $g(s)$ throughout the adiabatic evolution. The behavior of the spectrum of interpolated Hamiltonians (such as $H(s)$) in the vicinity of an avoided crossing has been studied very soon after the first version of the adiabatic theorem \cite{born1928beweis}. For details, see \cite{arthurthesis} for an exposition of these results. However, such techniques do not yield tight bounds for the gap, away from the avoided crossing. In our work, we achieve this by using a variety of techniques, namely (i)~the variational principle, (ii)~methods used in the context of continuous-time quantum walks \cite{childs2004spatial, chakraborty2016spatial, chakraborty2020optimality}, and (iii) the Sherman-Morrison formula \cite{sherman_morrison}. We also develop a simplified version of the adiabatic theorem, which requires fewer assumptions than the prior art \cite{jansen2007bounds}. This extends the recent work of Ref.~\cite{cunningham2024eigenpath} to the continuous-time setting. Our general results can be used to obtain the time after which adiabatically evolving a state under Hamiltonian $H(s)$ with a local schedule results in a state having a high fidelity with the ground state of $H(1)$. These methods are more broadly applicable, and the details of the same will appear elsewhere \cite{cunningham2025forthcoming}. Here, we obtain upper bounds on the running time of AQO by applying the aforementioned results.

In order to prove the \sharpP-hardness of predicting the position of the avoided crossing, we use polynomial interpolation and ideas that have been used in the context of theoretical proposals for demonstrating quantum supremacy, such as IQP sampling \cite{bremner2017achievingquantum}, Boson sampling \cite{aaronson2011bosonsampling}, and random circuit sampling \cite{bouland2019complexity, movassagh2023hardness}. In fact, we show that predicting the quantity that approximates the position of the avoided crossing exactly is as hard as predicting the output probability distribution of IQP circuits. To prove that it is also \sharpP-hard to approximate this quantity (up to a precision of $2^{-\poly(n)}$), we make use of polynomial interpolation, Paturi's lemma \cite{paturi1992}, and the Berlekamp-Welch algorithm \cite{bouland2019complexity, movassagh2023hardness}, which has also been central to prove the approximate hardness of random circuit sampling \cite{bouland2019complexity, movassagh2023hardness}, one of the candidate proposals for demonstrating quantum supremacy. 
...
Consider the positive constant $c\leq 0.242$ defined in Definition \ref{def:prob-Ham}, and the adiabatic Hamiltonian $H(s)$ in Eq.~\eqref{eq:def-H-sigma-bar}. Furthermore, define
$$
...
\end{align}
% \begin{align}
% g(s)&\leq O\left(\dfrac{s(A_1+1)}{A_2(1-s)}\sqrt{\left(\dfrac{A_1}{A_1+1}-s\right)^2+\dfrac{4A_2d_0}{N(A_1+1)^2}(1-s)^2}\right)
% \end{align}
% Observe that at $s=s^*+\delta_s$, we have
% \begin{align}
% g(s)&\leq O\left(\dfrac{s(A_1+1)}{A_2(1-s)}\sqrt{\delta^2_s+\delta^2_s (1-s)^2 (1+A_1)^2}\right)\\
% &\leq O\left(\dfrac{A_1}{A_1+1}\sqrt{\dfrac{d_0}{A_2 N}}\sqrt{\dfrac{s^2}{A_1^2(1-s)^2}+\dfrac{s^2 (1+A_1)^2}{A^2_1}}\right)\\
%     &\leq O\left(\dfrac{A_1}{A_1+1}\sqrt{\dfrac{d_0}{A_2 N}}\Bigg[1+\delta_s(1+1/A_1)\Bigg]\sqrt{1+\dfrac{1}{(1-\delta_s(1+A_1))^2}}\right)
% \end{align}
% We have that for $s\in \mathcal{I}_{s^*}$, $\delta_s\ll s^*$ and also $\delta_s\ll 1/(A_1+1)$. Thus, for any $s\in \mathcal{I}_{s^*}$,
% \begin{align}
% g(s)&\leq O\left(\dfrac{A_1}{A_1+1}\sqrt{\dfrac{d_0}{A_2 N}}\right).
% 	\label{eq:gap-scaling-robustness-window}
% \end{align} 
\end{proof}
...
~\\~\\
\textbf{Bounding the spectral gap to the right of the avoided crossing:~}Obtaining a tight bound on the spectral gap to the right of the avoided crossing is considerably more challenging. This is because the spectrum is more complicated in this region (See Fig.~\ref{fig:spectrum-H(s)}), and we require a different technique to keep track of the gap in this region. We consider a straight line $\gamma(s)$ from $sE_0+k~g_{\min}$ (at $s=s^*$) to some point $a$ between the two lowest eigenvalues of $H_z$ at $s=1$, where $k<1$ is a non-trivial constant and $a$ controls the slope of this line. The gap $g(s)$ is then lower bounded by twice the inverse of the norm of the resolvent of $H(s)$ with respect to $\gamma(s)$, denoted by $R_{H(s)}(\gamma)$. We obtain an upper bound on the norm of resolvent by the Sherman-Morrison formula \cite{sherman_morrison}. Additionally, by carefully tuning $k$ and $a$, we obtain a good enough lower bound on $g(s)$ for any $s\geq s^*$ (including the interval $\mathcal{I}_{s^{\rightarrow}}=(s^*+\delta_s, 1]$). We can also prove that  $g_{\min}$ is a lower bound on the spectral gap of $H(s)$ in this interval. Formally, we state the following lemma:
\begin{lemma}
...
\beta(s)=a\left(\dfrac{s-s_0}{1-s_0}\right).
$$
We do not define $a$ or $k$ at the moment and choose their appropriate values later.

The spectral gap of $H(s)$ is at least twice the minimum distance of $\gamma(s)$ from the spectrum of $H(s)$. That is,
\begin{equation}
g(s)\ge\dfrac{2}{\norm{R_{H(s)}(\gamma)}},
\end{equation}
...
In the previous sections, we have demonstrated that adiabatic quantum optimization can find the ground states of any Hamiltonian (having a sufficiently large spectral gap and diagonal in the computational basis) with a quadratic speedup over unstructured ``brute force" classical search. However, prior knowledge of the position of the avoided crossing was necessary for the optimality of the algorithm. Here, we show that the problem of estimating $A_1$ exactly or to a precision of $O(2^{-\poly(n)})$ is \sharpP-hard.

For this section, we change the notation slightly for brevity. Consider any $n$-qubit Hamiltonian $H$ diagonal in the computational basis having distinct eigenlevels with eigenenergies $E_0<E_1<\cdots E_{M-1}$, such that each eigenlevel with eigenvalue $E_k$ is $d_k$-fold degenerate. Then, we relabel the spectral parameter $A_1$ (defined in Eq.~\eqref{eq:spectral-parameters}) as $A_1(H)$ for convenience. That is,

\begin{equation}
A_1(H)=\dfrac{1}{2^n}\sum_{k=1}^{M-1}\dfrac{d_k}{\Delta_k}, 
\end{equation}
where $\Delta_k=E_k-E_0$. Furthermore, for our analysis in this section, we do not normalize $H$. Assuming that $\norm{H}$ is known (which for our parameter range is in $O(\poly(n))$), all our results will also hold for normalized Hamiltonians, with a simple rescaling by this factor. 


We first prove the claim that it is \sharpP-hard to estimate $A_1$ exactly. For this, consider the Ising Hamiltonian 
\begin{equation}
...
\end{lemma}
Lemma \ref{lem:exact-degeneracy-hard} and Lemma \ref{lem:approx-degeneracy-hard} combined, prove Theorem \ref{thm:main-result-2}.
...
Unfortunately, these proof techniques based on polynomial interpolation do not allow us to conclude anything about the hardness of the approximation of $A_1(H)$ up to the additive error tolerated by the adiabatic algorithm described in Sec.~\ref{sec:poac-and-schedule}.  We remark that limitations of hardness proofs based on polynomial interpolation were also identified in the context of proposals for demonstration of quantum computational advantage such as boson sampling \cite{aaronson2011bosonsampling} and random circuit sampling \cite{bouland2019complexity}, leading to conjectures about the hardness of approximating outcome probabilities from these circuits which still remain open. From the statement of Theorem \ref{thm:main-result-1}, we require an algorithm that estimates $A_1$ to additive accuracy $O(\delta_s)$ in time at most $\widetilde{O}(\sqrt{2^n/d_0})$. However, the best classical algorithms for approximating the solutions to \sharpP-complete problems leverage sampling-based techniques and have a running time of $O(1/\delta^2_s)=\widetilde{O}(2^n/d_0)$ \cite{gurvitz2005mixed}. One might consider using sampling-based methods to estimate $A_1$ to the desired precision. For instance, given a Hamiltonian $H$, suppose there exists a procedure that efficiently samples the inverse gaps $\Delta_k$ according to the probability distribution $\{d_k/2^n\}_{k=1,\ldots, M-1}$. Then, how many queries to this procedure are required to estimate the expectation value of this distribution (which is $\mu=A_1$) to a precision of $\delta_s$? Note that the variance is $\sigma^2=A_2-A_1^2\leq A_2$. Then, by using Chebyshev's inequality, it is easy to see that at least $\widetilde{O}(2^n/d_0)$ queries are needed to estimate $A_1$ to a precision of $\delta_s$, with a constant probability. 

On the other hand, it is possible to prove that even if we relax the precision up to which $A_1$ is estimated, the problem is already \NP-hard. In this case, we consider the $\mathsf{MaxCut}$, a well-known \NP-complete problem. Given a simple unweighted graph $G(V,E)$, of $|V|=n$ vertices and $|E|$ edges, a \textit{cut} is defined as a subset $S$ of $V$ that partitions the entire vertex set into $S$ and $\bar{S}=V\backslash S$. Then, the size of the cut is the number of edges from the vertices in $S$ to the vertices in $\bar{S}$, i.e.\ $|E(S,\bar{S})|$. Given a graph $G$ and a number $k>0$, the $\mathsf{MaxCut}$ problem asks if there exists a cut of size at least $k$. This decision version of the problem can also be formulated in terms of whether it is possible to disambiguate between two thresholds of the ground energy of a Hamiltonian encoding $G$. More precisely, given $G$, it is possible to define the Hamiltonian
\begin{equation}
    H=|E|\cdot I -\sum_{(i,j)\in E}\frac{I-\sigma_z^i\sigma_z^j}{2},
\end{equation} 
with ground energy $E_0$ such that solving $\mathsf{MaxCut}$ (asking if there is a cut of size at least $k$) is equivalent to distinguishing between:~(i) If $E_0\geq |E| - k +1$ or (ii) $E_0\leq |E| - k$. Now, suppose there exists a classical algorithm that approximates $A_1$ to an additive accuracy of $\varepsilon$. Then, in the Appendix, we rigorously prove that it is possible to disambiguate between (i) and (ii) by making only two calls to this algorithm, provided $\varepsilon< 1/\poly(n)$ (See Lemma \ref{lem:np-hardness-maxcut} of Sec.~\ref{sec-app:np-hardness-A1}). Thus, the problem of approximating $A_1$ to a much larger value of precision is already \NP-hard. This adds to the evidence that it is computationally hard to estimate $A_1$ within an accuracy of $O(\delta_s)$ in time $O(2^{n/2})$.
...
% \sharpP\ hardness for estimating $A_1$ exactly (or to a precision of $O(2^{-\poly(n)})$) also impacts other (non-adiabatic) continuous-time quantum algorithms for solving optimization problems. 

A natural question to ask is whether this issue persists for the standard choice of Hamiltonians in adiabatic quantum optimization algorithms, i.e.\ when the adiabatic Hamiltonian interpolates between an initial Hamiltonian that is a local sum of spins, given by \ $H_0=\sum_{j} \sigma^j_x$ (instead of a one-dimensional projector), and an Ising Hamiltonian $H_{\sigma}$. That is, 
\begin{equation}
\label{eq:adiabatic-ham-local-sigma-x}
    H(s)=-(1-s)\sum_j \sigma^j_x + s H_{\sigma}. 
\end{equation} 
It has been observed that when $H_{\sigma}$ encodes \NP-complete problems, the spectrum of $H(s)$ is extremely complicated, and analytical results are largely absent. For some random instances of \NP-complete problems, it has been observed that the spectrum of $H(s)$ has exponentially many avoided crossings with exponentially small gaps \cite{altshuler2010anderson}. This makes it significantly more challenging to handle this problem analytically and derive a provable running time. Our results demonstrate that even in a simpler setting (such as unstructured search), where the underlying Hamiltonian is guaranteed to have only a single avoided crossing, the position of this avoided crossing can be hard to approximate. 

Our work points to fundamental open problems concerning AQO. For instance, is it possible to use AQO to obtain generic quadratic speedups for solving optimization problems using a purely adiabatic procedure without having to solve a computationally hard problem to determine the appropriate schedule? One possible direction could be to modify the adiabatic Hamiltonian itself, for example, by expanding the dimension of the Hamiltonian (by adding qubits) and/or introducing intermediate Hamiltonians at various points in the adiabatic path. These strategies can potentially shift the position of the avoided crossing to a point independent of the spectrum of the problem Hamiltonian. 

%Furthermore, it would be interesting to quantify the hardness of approximating the position of the avoided crossing to the desired precision. In this article, we have proven that it is \sharpP-hard to estimate this quantity exactly, or to a precision of $2^{-\poly(n)}$. It is plausible to conjecture that when the additive precision scales as $2^{-n/2}$, the problem remains \sharpP-hard. This is because we prove it is already \NP-hard to approximate the quantity to a much larger precision of $1/\poly(n)$. This might require invoking new techniques that go beyond polynomial interpolation.
...
Suppose $H_z$ be any $n$-qubit Hamiltonian, diagonal in the computational basis, with distinct eigenvalues $-1\leq E_0< E_1 < E_2<\cdots< E_{M-1}\leq 1$, such that $E_k$ has degeneracy $d_k$, satisfying $\sum_{k=0}^{M-1} d_k=2^n=N$. Also suppose for a small positive constant $c\leq 0.242$, the spectral gap $\Delta$ of $H_z$ satisfies
...
&\delta_-(s)\in\left((1+\eta)\delta^{-}_{0}(s),~(1-\eta)\delta^{-}_{0}(s)\right),
\end{align}
where, $\eta$ is a small constant ($\leq c$), and
\begin{equation}
\delta^{\pm}_{0}(s)=\dfrac{s(A_1+1)}{2A_2(1-s)}\left[~s-\dfrac{A_1}{A_1+1}\right.\left.\pm\sqrt{\left(\dfrac{A_1}{A_1+1}-s\right)^2+\dfrac{4A_2d_0}{N(A_1+1)^2}(1-s)^2}~\right]. 
\end{equation}
\end{lemma}
...
\end{equation}
such that $F(\delta)=0$ is equivalent to Eq.~\eqref{eq:delta-s-full}. Then we consider the Taylor expansion of the summation term in $F(\delta)$ in terms of $\delta$ to obtain
\begin{equation}
\label{eq:expression-F-delta}
F(\delta)=\frac{d_0}{N\delta}\left[-1+\frac{N\delta}{d_0s}\left(A_1-\frac{s}{1-s}\right)\right.\\\left.+\frac{NA_2\delta^2}{d_0s^2}+f(\delta)\right]
\end{equation}
where the remaining terms, 
\begin{equation}
f(\delta)=\frac{N}{d_0}\sum_{p=3}^\infty\frac{A_p\delta^p}{s^p}
\end{equation}

Observe that $\delta_0^\pm$ are the roots of the quadratic equation formed by ignoring the terms in $f(\delta)$, and thus we show that $|f(\delta)|$ is upper-bounded by some small constant. Since $\Delta$ is the spectral gap of $H_z$, we have that for all any integers $p\ge q\geq 1$,
\begin{equation}\label{eq:A-pq-inequality}
A_p\le\frac{A_q}{\Delta^{p-q}}.
\end{equation}

So, using this inequality for $q=3$, along with the extended triangle inequality, we  obtain

\begin{equation}
\label{eq:upper-bound-f-delta}
|f(\delta)|\le\frac{NA_3|\delta|^3}{d_0s^3}\sum_{p=0}^\infty\frac{|\delta|^p}{(s\Delta)^p}
\end{equation}
...
\begin{align}
    |\delta^\pm_0(s)|&\leq\dfrac{s(A_1+1)}{2 A_2 (1-s)}\left(\delta_s+\sqrt{\delta^2_s+(A_1+1)^2(1-s)^2\delta^2_s}\right)\\
    &\leq s \dfrac{(A_1+1)^2}{2A_2}\delta_s \left[\frac{1}{(1-s)(1+A_1)}+\sqrt{\frac{1}{(1-s)^2(1+A_1)^2}+1}~\right]  \\
    &\leq s\Delta c \left[\frac{1}{1-\frac{\delta_s}{1-s^*}}+\sqrt{\frac{1}{(1-\frac{\delta_s}{1-s^*})^2}+1}~\right] \\
    &\leq s\cdot \Delta \cdot c \left(\dfrac{1+\sqrt{1+(1-2c)^2}}{(1-2c)}\right)\\
				&\leq  s\cdot \Delta \cdot c \cdot\kappa
				\label{eq:upper-bound-delta_0}
\end{align}
Here, we used Eq.~\eqref{eq:hamiltonian-constraint} with the fact that 
$$\frac{(A_1+1)^2}{2A_2}\delta_s = \Delta \frac{1}{\Delta} \sqrt{\frac{d_0}{A_2N}}<\Delta c.$$ 
Also, from the proof of Lemma \ref{lem:spectral-gap-in-robustness-window}, we have shown that $\frac{\delta_s}{1-s^*} \leq 2c$. We introduced
$$
\kappa = \left(\dfrac{1+\sqrt{1+(1-2c)^2}}{(1-2c)}\right),
$$
such that $c'=c\kappa$ with $c\leq 0.242$ ensures that $c'<1$. This leads to
\begin{equation}
\dfrac{\left|\delta^\pm_0(s)\right|}{s\Delta}\le c',
\end{equation}


Consequently, substituting this upper bound in Eq.~\eqref{eq:upper-bound-f-delta} allows us to obtain
\begin{equation}
\left|f(\delta^{\pm}_0)\right|\le\dfrac{NA_3\left|\delta^{\pm}_0\right|^3}{d_0s^3}\dfrac{1}{1-c'}.
\end{equation}

Substituting the upper bound on $|\delta^\pm_0|$ from Eq.~\eqref{eq:upper-bound-delta_0}, into this equation gives us for any $\mu$,
\begin{equation}
\label{eq:upper-bound-f-delta-1+mu}
\left|f\left(\delta^\pm_0\left(1+\mu\right)\right)\right|\leq \dfrac{A_3}{A_2^{\frac{3}{2}}}\sqrt{\dfrac{d_0}{N}}\cdot\dfrac{\kappa^3(1+\mu)^3}{1-c'}.
\end{equation}

Now define 
$$
\eta=\dfrac{A_3}{A_2^{3/2}}\sqrt{\dfrac{d_0}{N}}, 
$$  
and observe that
\begin{equation}
\eta=\dfrac{1}{\Delta}\sqrt{\frac{d_0}{A_2 N}}\cdot\dfrac{A_3\Delta}{A_2}
\end{equation}
From the condition in Eq.~\eqref{eq:hamiltonian-constraint} and the inequality stated in Eq.~\eqref{eq:A-pq-inequality} for $p=3,q=2$ we derive that $\eta<c$, where $c$ is a small constant.

Then by substituting $\mu=\eta$ in Eq.~\eqref{eq:upper-bound-f-delta-1+mu}, we obtain
\begin{equation}
\left|f(\delta^\pm_0(1+\eta))\right|\leq \eta \dfrac{\kappa^3(1+\eta)^3}{1-c'}\leq \eta (1+\eta)\dfrac{\kappa^3(1+c)^2}{1-c'}.
\end{equation}
This proves that $|f(\delta^\pm_0(1+\eta'))|$, where $\eta'=\pm\eta$, is bounded by a small constant for any $s\in\mathcal{I}_{s^*}$. Finally, substituting 
$$
\delta=\delta^\pm_0(1+\eta'),
$$ 
in Eq.~\eqref{eq:expression-F-delta}, we obtain
\begin{equation}
...
%%%%%%
\noindent Therefore, we have the following bounds on $F(\delta^+_0(1+\eta'))$:
\begin{align}
    F(\delta^+_0(1+\eta'))&\geq \dfrac{d_0}{N\delta^+_0}\eta\left[\text{sign}(\eta')+\frac{\text{sign}(\eta')}{1+\eta'}+\text{sign}(\eta')\left(\dfrac{s-s^*}{1-s^*}\right)\frac{N}{d_0}\dfrac{\delta^+_0}{s(1-s)}-\dfrac{\kappa^3(1+c)^2}{1-\kappa c}\right] \label{eq:lbound_Fd0+}\\
    F(\delta^+_0(1+\eta'))&\leq \dfrac{d_0}{N\delta^+_0}\eta\left[\text{sign}(\eta')+\frac{\text{sign}(\eta')}{1+\eta'}+\text{sign}(\eta')\left(\dfrac{s-s^*}{1-s^*}\right)\frac{N}{d_0}\dfrac{\delta^+_0}{s(1-s)}+\dfrac{\kappa^3(1+c)^2}{1-\kappa c}\right] \label{eq:ubound_Fd0+}
\end{align}
Then, we use Eq. (\ref{eq:lbound_Fd0+}) with $\eta'=\eta$ to show that $F(\delta^+_0(1+\eta))$ is positive and Eq. (\ref{eq:ubound_Fd0+}) with $\eta'=-\eta$ to show that $F(\delta^+_0(1+\eta))$ is negative under the following constraints on $c$:
\begin{align}
    \frac{3}{2}-\dfrac{\kappa^3(1+c)^2}{1-\kappa c} \geq \frac{2\kappa}{1-2c} \\
    2-\dfrac{\kappa^3(1+c)^2}{1-\kappa c} \geq \frac{2\kappa}{1-2c}
\end{align}
where we observed that $\left|\frac{s-s^*}{1-s^*}\right|\frac{N}{d_0}\frac{\delta^+_0}{s(1-s)} \leq \frac{2 \kappa}{1-2c}$.
These two constraints give an \textbf{OPPOSITE} bound on $c$ that we already have i.e.\ they are satisfied as long as $c\geq 0.242$.
\\

A similar reasoning holds for $\delta_0^-$. From this we conclude that the roots of $F(\delta)=0$ exist in the intervals \begin{align*}
&\delta_+(s)\in\left((1-\eta)\delta^{+}_{0}(s),~(1+\eta)\delta^{+}_{0}(s)\right),~\text{ and }\\
&\delta_-(s)\in\left((1+\eta)\delta^{-}_{0}(s),~(1-\eta)\delta^{-}_{0}(s)\right),
\end{align*}
where, $\eta$ is a small constant ($\leq c$), and
...
\end{proof}
\section{Gap to the right of the avoided crossing: Proof that $f(s)$ is monotonically decreasing}
...
In this section, we shall make use of the recent results in \cite{cunningham2024eigenpath}, which provides a rigorous version of the adiabatic theorem while requiring minimal assumptions on $H(s)$. The results allow us to obtain the running time for adiabatic quantum computation involving any Hamiltonian that is twice differentiable such that there exists some bound on its spectral gap $g(s)$. Thus, we are able to apply these general results to the AQO problem. In this section, we derive the running time of adiabatic quantum computation for a generic $H(s)$ that satisfies the aforementioned conditions. In the next section, we apply the results developed here to obtain the running time of our algorithm (Theorem \ref{thm:main-result-1}). We begin by outlining the assumptions on $H(s)$ for the results to hold. 
...
Consider any parametrized Hamiltonian $H(s)$ that satisfies Definition \ref{def:adiabatic-ham-general} and $K: [0,1]\to \mathbb{R}^+$, a differentiable function such that the derivative $K'$ is absolutely continuous. Then, the evolution of any state $\rho$ under $H$, according to the schedule $K(s)$ satisfies
$$
...
In this section, we leverage results from Sec.~\ref{eq:adiabatic-epath-traversal} to provide a generic upper bound on the running time $T$ when such a local schedule is employed to perform adiabatic quantum computation. More precisely, the following theorem shows that adiabatic evolution under a local schedule for a time $T$ results in a final state that has at least $1-\varepsilon$ with the target state (for instance, the ground state of the problem Hamiltonian).
...
Here, we provide detailed proof of one of our main results, namely the running time of the adiabatic algorithm. For this, we borrow results from Sec.~\ref{sec-app:adiabatic-theorem}.
...
\end{proof} 
\section{NP-hardness of estimating $\mathbf{A_1}$ to low precision}
\label{sec-app:np-hardness-A1}
In this section, we shall prove that estimating the spectral parameter $A_1$ to a low precision is already \NP-hard. More precisely, suppose there exists a classical procedure that accepts the description of a classical Hamiltonian $H$ (diagonal in the computational basis) and outputs $A_1$ to a precision of $O(1/\poly(n))$. Then, by making only a constant number of calls to this procedure, it is possible to solve $\mathsf{MaxCut}$, a well-known \NP-Complete problem.

For completeness, we begin by discussing the $\mathsf{MaxCut}$ problem briefly. Consider a graph $G(V,E)$ of $|V|=n$ vertices and $|E|$ edges \footnote{For simplicity, we restrict our discussion to simple unweighted graphs, although the $\mathsf{MaxCut}$ problem can also be defined for weighted graphs}. A \textit{cut} in $G$ is a subset $S\subseteq V$. More precisely, if $\bar{S}$ is the set of vertices other than those in $S$, i.e.\ $\bar{S}=V\backslash S$, then the cut $S$ partitions the vertex set into $S$ and $\bar{S}$. Moreover, the size of the cut $S$ is the number of edges from vertices in $S$ to the vertices in $\bar{S}$, i.e.\ $|E(S,\bar{S})|$. The $\mathsf{MaxCut}$ problem then asks to find the cut $S$ that maximizes $|E(S,\bar{S})|$. It is well known that $\mathsf{MaxCut}$ is \NP-Complete and can be posed as a decision problem as well as an optimization problem. Given a graph $G(V,E)$ and an integer $k$, the decision version of this problem asks whether there is a cut of size at least $k$. This is equivalent to the following setting. Given $G(V,E)$, consider the Hamiltonian
\begin{equation}
\label{eq:max-cut-ham}
    H=-\sum_{(i,j)\in E}\frac{I-\sigma_z^i\sigma_z^j}{2}.
\end{equation} 
Note that the largest eigenvalue $E_{M-1}$ is $0$ while the smallest $E_0\geq -|E|$. Then solving $\mathsf{MaxCut}$ is equivalent to distinguishing between the following two thresholds of the ground energy $E_0$:
\begin{itemize}
    \item[(i)]~$E_0 \geq - k +1$
    \item[(ii)]~$E_0\leq -k$
\end{itemize}
Now suppose there exists a classical procedure $\mathcal{C_{\varepsilon}}$ that accepts as input the description of a classical Hamiltonian $H$ and outputs a quantity $\tilde{A}_1$, which approximates $A_1$ to within an additive accuracy $\varepsilon$. Then, in what follows, we prove that it is possible to solve $\mathsf{MaxCut}$ by making only two queries to $\mathcal{C}_{\varepsilon}$, provided $\varepsilon\leq 1/\poly(n)$. Note that this is significantly larger than the precision with which $A_1$ needs to be approximated for AQO (which is roughly $2^{-n/2}$). It is thus plausible to conjecture that estimating $A_1$, and hence the position of the avoided crossing to the desired accuracy, is computationally hard. Concretely, we prove the following lemma:
\begin{lemma}
\label{lem:np-hardness-maxcut}
Let $\varepsilon\in (0,1)$. Suppose there exists a classical procedure $\mathcal{C}_{\varepsilon}(\langle H \rangle)$ that accepts the description of a Hamiltonian $H$ and outputs $\tilde{A}_1(H)$ such that
$$
\left|\tilde{A}_1(H)-A_1(H)\right|\leq \varepsilon.
$$
Now given a simple unweighted graph $G(V,E)$ of $|V|=n$ vertices and $|E|$ edges, it is possible to solve the $\mathsf{MaxCut}$ problem, i.e.\ decide whether there exists a cut of size at least $k$ in $G$ by making only two calls to $\mathcal{C}_{\varepsilon}$, provided
$$
\varepsilon < \dfrac{2}{3}\cdot \dfrac{1}{n^4}.
$$
\end{lemma}
\begin{proof}
We consider the Hamiltonian $H$, defined in Eq.~\eqref{eq:max-cut-ham} for $G(V,E)$ of $|V|=n$ vertices and $|E|$ edges. Suppose, the distinct eigenenergies of $H$ are $-|E|\leq E_0<E_1<\ldots<E_{M-1}=0$, such that eigenvalue $E_k$ is $d_k$-degenerate. Then, as described earlier, solving $\mathsf{MaxCut}$ (if a cut of size at least $k$ exists) is equivalent to distinguishing between (i) If $E_0\geq - k +1$ or (ii) $E_0\leq - k$. We prove that this can be done by making only two calls to $\mathcal{C}_{\varepsilon}$.

For this, we consider the Hamiltonian $H'$ (obtained from $H$), defined as follows:
$$
H'=H - \frac{1+\sigma_z^{(1)} \sigma_z^{(n+1)}}{2}.
$$ 
Let us first compare $A_1(H)$ with $A_1(H')$:
\begin{align*}
A_1(H) &= \frac{1}{2^n}\sum_{k\neq 0} \frac{d_k}{E_k-E_0} \\
&= \frac{1}{2^n}\sum_{k\neq 0} \frac{d_k}{E_k} + \frac{E_0}{2^n}\sum_{k\neq 0} \frac{d_k}{E_k(E_k-E_0)} \\
&= 2A_1(H') - \frac{d_0}{2^n}\frac{1}{E_0} + \frac{E_0}{2^n}\sum_{k\neq 0} \frac{d_k}{E_k(E_k-E_0)} \\
&\geq 2A_1(H') + \frac{E_0}{E_{M-1}}\frac{1}{(E_{M-1}-E_0)}\left( 1-\frac{d_0}{2^n}\right)- \frac{d_0}{2^n} \frac{1}{E_0}\\
&\geq 2A_1(H') + \underbrace{ \left (\frac{1}{E_{M-1}-E_0}-\frac{1}{E_{M-1}}\right )\left( 1-\frac{d_0}{2^n}\right)- \frac{d_0}{2^n} \frac{1}{E_0}}_{=K}
\end{align*}
Now we look at $K$ in the two different cases, using the fact that $E_M=|E|$.
~\\~\\
\textbf{(i)~$\mathbf{E_0 \geq |E|-k+1}$:~}We have:
\begin{align}
    K\geq \left ( \frac{1}{k-1}-\frac{1}{|E|}\right)\left ( 1-\frac{d_0}{2^n}\right)-\frac{d_0}{2^n} \frac{1}{|E|-k+1}=K_1
\end{align}

\noindent \textbf{(ii)~$\mathbf{E_0 \leq |E|-k}$:~}In this case:
\begin{align}
    K\leq \left ( \frac{1}{k}-\frac{1}{|E|}\right)\left ( 1-\frac{d_0}{2^n}\right)-\frac{d_0}{2^n} \frac{1}{|E|-k}=K_2
\end{align}
Now, by querying $\mathcal{C}_{\varepsilon}$ twice, first with $H$ and subsequently with $H'$ as inputs, we obtain $\tilde{A}_1(H)$ and $\tilde{A}_1(H')$ respectively. Then, we can disambiguate between the two cases by making two calls to algorithm $\mathcal{C}_{\varepsilon}$ if 
\begin{align}
    K_1-3\varepsilon > K_2 +3\varepsilon,
\end{align}
which implies
\begin{align}
    6\varepsilon < \frac{1}{k(k-1)}\left(1-\frac{d_0}{2^n}\right)+\frac{d_0}{2^n} \frac{1}{(|E|-k+1)(|E|-k)}
\end{align}
Note that we have $1\leq k \leq |E|$, which implies that the additive precision $\varepsilon$ with which $A_1$ is estimate is at most inverse polynomial in $n$. Indeed, it suffices if
\begin{align}
    \varepsilon < \frac{1}{6}\frac{1}{|E|(|E|-1)} 
\end{align}
Recall for any simple, unweighted graph of $n>0$ nodes, $|E|\leq n(n-1)/2$, and so 
$$
\dfrac{1}{6}\dfrac{1}{|E|(|E|-1)} \geq \dfrac{2}{3}\dfrac{1}{n(n-1)(n-2)(n+1)} \geq \dfrac{2}{3}\dfrac{1}{n^4}.
$$
Finally, this implies for solving $\mathsf{MaxCut}$, it suffices to make two queries to $\mathcal{C}_{\varepsilon}$, provided
\begin{align}
    \varepsilon<\frac{2}{3}\frac{1}{n^4}.
\end{align}
This completes the proof.
\end{proof}
%
...
%         \includegraphics[width=1\linewidth,trim={6cm 0 6cm 0}, clip]{plot_ACinAQC002.jpeg}
%         \caption{~}
%         \label{fig:spectrum-H(s)-overall}
%     \end{subfigure}%
%     \begin{subfigure}{.5\textwidth}
%         \includegraphics[width=1\linewidth,trim={6cm 0 6cm 0}, clip]{plot_ACinAQC003.jpeg}
%         \caption{~}
%         \label{fig:spectrum-H(s)-avoided-crossing}
%     \end{subfigure}
%     \caption{Spectrum of the adiabatic Hamiltonian of $n=20$ qubits, denoted by $H(s)=-(1-s)\ket{\psi_0}\bra{\psi_0}+sH_z$, where the schedule $s\in [0,1]$. The initial Hamiltonian is a one-dimensional projector such that $\ket{\psi_0}$ is an equal superposition of all computational basis states of $n$ qubits. The final Hamiltonian $H_z$ is diagonal in the computational basis such that there are $11$ distinct eigenenergies $E_k$, equally spaced between $0$ and $1$. Each eigenvalue $E_k$ is $d_k$-degenerate, where the degeneracies are distributed according to a Gaussian. In (a), the vertical dotted line (red) denotes the position of the avoided crossing between the ground and the first excited state. The ground and the first excited states are isolated from the spectrum of $H(s)$ in (b). The region of the avoided crossing corresponds to a window of width $\delta_s$ on either side of $s^*$. In the article, we prove that the gap between the ground state and the first excited state in this region scales as the minimum spectral gap of $H(s)$, denoted by $g_{\min}$ in the article. In (b), the lower bound we obtained on the spectral gap of $H(s)$ (denoted as $g(s)$) to the left of the avoided crossing region is also shown. For this, we obtain an upper bound on the ground energy $\lambda_0(s)$ and a lower bound on the energy of the first excited state, $\lambda_1(s)$. We consider an ansatz $\ket{\phi}$ and use the variational principle to upper bound the ground energy $\lambda_0(s)\leq \braket{\phi|H(s)|\phi}$ (depicted by pink dotted lines). Additionally, $\lambda_1(s)$ is lower bounded by $sE_0$, which is depicted by a black horizontal dotted line.}
%     \label{fig:spectrum-H(s)}
% \end{figure}


Suppose $H$ is a normalized Hamiltonian with the promise that its ground state energy is either zero or above a certain threshold $\mu$. The goal is to disambiguate between these two cases. This is the well-known \textit{local Hamiltonian problem}. If $H$ is a quantum Hamiltonian (sum of non-commuting local terms that is not necessarily diagonal in the computational basis), the \textit{local Hamiltonian problem} is \QMA-complete. In our case $H$ is classical, i.e.\ it is diagonal in the computational basis. For classical Hamiltonians, this problem is known to be \NP-Hard as long as the promise gap $\mu$ is at least $1/\poly(n)$. Now suppose there is a classical procedure $\mathcal{C}_{\varepsilon}$ that accepts the description of a Hamiltonian $H$ and outputs an estimate of $A_1$ to additive accuracy $\varepsilon \leq 1/\poly(n)$. Then it is possible to disambiguate between the two thresholds of the ground energy $E_0$ of $H$ with a promise gap of at least $1/\poly(n)$, by making only $O(\poly(n))$ calls to $\mathcal{C}_{\varepsilon}$. Since $H$ can correspond to an \NP-complete problem such as \MaxSAT or 3-\SAT, the problem of estimating $A_1$ to a precision of $1/\poly(n)$ is already \NP-Hard. Note that this is significantly lower than the precision with which $A_1$ needs to be approximated for AQO (which is roughly $2^{-n/2}$). It is thus plausible to conjecture that estimating $A_1$, and hence the position of the avoided crossing to the desired accuracy, is computationally hard. We begin by proving the following lemma:
~\\
\begin{lemma}
Let $\varepsilon,~\mu\in (0,1)$ and $\mu>0$. Suppose there exists a classical procedure $\mathcal{C}_{\varepsilon}(\langle H \rangle)$ that accepts the description of a Hamiltonian $H$ and outputs $\tilde{A}_1(H)$ such that
$$
\left|\tilde{A}_1(H)-A_1(H)\right|\leq \varepsilon.
$$
Now consider any $n$-qubit Hamiltonian $H$, diagonal in the computational basis such that it has eigenvalues $0\leq E_0< E_1<\ldots < E_{M-1}\leq 1$ such that $M\in \poly(n)$, and eigenvalue $E_k$ has degeneracy $d_k$. Furthermore, suppose that the ground energy $E_0$ of $H$ satisfies the following: Either (i)~$E_0=0$ or (ii)~$\mu\leq E_0\leq 1-\mu$. Then, it is possible to decide whether (i) or (ii) holds, by making two calls to $\mathcal{C}_{\varepsilon}$, provided
$$
\varepsilon < \dfrac{\mu}{6(1-\mu)}-\dfrac{d_0}{6~2^n}\cdot\dfrac{1}{\mu^2}
$$
\end{lemma}
\begin{proof}
Given a description of $H$, we aim to disambiguate between $E_0 = 0$ and $\mu\leq E_0 \leq 1-\mu$ by querying $\mathcal{C}$ twice. First, consider the Hamiltonian 
$$
...
A_1(H)-2A_1(H')=0.
$$
\textbf{When $\mathbf{E_0\neq 0}$:~} In this case the ground energy of $H'$ is zero with degeneracy $2^n$. The Every other distinct eigenlevel has energy $E'_k=E_{k-1}$ having degeneracy $d'_k=d_{k-1}$, where $1\leq k \leq M$. Therefore,
$$
...
&\geq 2A_1(H') + \dfrac{\mu}{1-\mu} - \dfrac{d_0}{2^n}\cdot \frac{1}{\mu^2}
\end{align}
Thus in this case, 
$$A_1(H) - 2A_1(H') \geq \frac{\mu}{1-\mu} - \frac{d_0}{2^n}\cdot \frac{1}{\mu^2}.
$$
...
\begin{equation}
\tilde{A}_1(H) - 2\tilde{A}_1(H') \geq \frac{\mu}{1-\mu} - \frac{d_0}{2^n}\cdot\frac{1}{\mu^2} - 3\varepsilon.
\end{equation}
In order to disambiguate between the two cases, we need
\begin{equation}
6\varepsilon < \frac{\mu}{1-\mu} - \frac{d_0}{2^n}\cdot\dfrac{1}{\mu^2},
\end{equation}
which completes the proof.
\end{proof}
\textcolor{blue}{I am here! Several things need to be made concrete. This, to me, is the \textit{local Hamiltonian problem}, when the underlying Hamitlonian is classical. This is known to be \NP-hard if the promise gap is $1/\poly(n)$. As it stands, the two outcomes ($E_0=0$ or $E_0\geq \mu$) are arbitrarily close, which shouldn't be the case. I suspect the two cases must separated by $1/\poly(n)$ for the hardness results to hold. Also, the other point is that we really need to make two queries to decipher the threshold of the ground energy. We don't even need $\poly(n)$ calls. What does this imply (other than the fact that this indicates it is much harder to approximate $A_1$ to the desired precision)?}